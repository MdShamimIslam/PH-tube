...
const handleTabClick = async (categoryId) =>{
    ...
    cardDiv.innerHTML = `
            ...
            <figure><img src="${perCategory.thumbnail}" alt="Shoes" /></figure>
            <div class="card-body">
                <div class="flex justify-start">
                    <img class="w-[40px] h-10 rounded-full" src="${perCategory?.authors[0]?.profile_picture}" alt="Shoes" />
                    <div class="ml-2">
                    <h2 class="card-title ml-2">${perCategory.title}</h2>
                    <div class="mt-2 flex">
                        <p>${perCategory?.authors[0]?.profile_name}</p>
                        <svg class="mr-[25px]" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
